// Import des fonctions utilitaires depuis supabaseUtils.js et de la bibliothèque puppeteer
import { getUrlArticle, testSupabaseConnection, enregistrerArticle, articleExists } from './supabaseUtils.js';
...
(async () => {
    ...
    // Décommenter la ligne suivante pour récupérer les URLs depuis Supabase
    const urls = await getUrlArticle(); // [{ url: "https://..." }, ...]
    ...
    const articles = await Promise.all(
        urls.map(e => limit(async () => {
            ...
            try {
                ...
                const existe = await articleExists(e.url);
                if (existe) {
